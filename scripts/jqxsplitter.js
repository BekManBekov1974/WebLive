  ...
  a.extend(a.jqx._jqxSplitter.prototype, {
    ...
    _initOverlay: function (b) {
      if (this.overlay || b == "undefined") {
        ...
        if (b == true) {
          this.overlay = a(
            "<div style='z-index: 100; background: #fff;'></div>"
          );
    ...
    _startDrag: function (b) {
      ...
      if (
        this.panels[0].collapsed ||
        this.panels[1].collapsed ||
        !this.resizable
      ) {
        return true;
      }
    ...
    _drag: function (b) {
      ...
      if (this._splitBarClone) {
        ...
        var f = 1 / d;
        var h = 0;
        var l = this._splitBarClone[k]() ;
        var g = parseInt(this.host.coord()[i]);
        var e =
          this._position[i] - this._dragStart[i] + this._cloneStart[i] - g;
        if (h > e) {
          e = h;
        }
        if (e > n + h - l) {
        ...
        this._splitBarClone.removeClass(
          this.toThemeProperty("jqx-splitter-splitbar-invalid")
        );
        if (e < j) {
          this._splitBarClone.addClass(
            this.toThemeProperty("jqx-splitter-splitbar-invalid")
          );
          e = j;
        }
    ...
    _addHandlers: function () {
      ...
      this.addHandler(this.splitBar, "dragstart." + this.element.id, function (
        e
      ) {
        return false;
      });
      if (this.splitBarButton) {
        ...
        this.addHandler(
          this.splitBarButton,
          this._eventName("mouseenter"),
          function () {
            c.splitBarButton.addClass(
              c.toThemeProperty("jqx-splitter-collapse-button-hover")
            );
            c.splitBarButton.addClass(
              c.toThemeProperty("jqx-fill-state-hover")
            );
          }
        );
        this.addHandler(
          this.splitBarButton,
          this._eventName("mouseleave"),
          function () {
            c.splitBarButton.removeClass(
              c.toThemeProperty("jqx-splitter-collapse-button-hover")
            );
            c.splitBarButton.removeClass(
              c.toThemeProperty("jqx-fill-state-hover")
      ...
      this.addHandler(
        a(document),
        this._eventName("mousemove") + "." + this.element.id,
        function (e) {
         return  c._drag(e);
      ...
      this.addHandler(
        a(document),
        this._eventName("mouseup") + "." + this.element.id,
        function () {
          return c._stopDrag();
        }
      );
      this.addHandler(this.splitBar, this._eventName("mousedown"), function (
        e
      ) {
      ...
      this.addHandler(
        ...
        function () {
          if (c.resizable && !c.disabled) {
            c.splitBar.addClass(
              c.toThemeProperty("jqx-splitter-splitbar-hover")
            );
    ...
    render: function () {
      ...
      if (c.length == 2) {
        ...
        if (
          e.indexOf("jqx-reset") != -1 &&
          e.indexOf("jqx-splitter") != -1 &&
          e.indexOf("jqx-widget") != -1
        ) {
        ...
        if (
          b.indexOf("jqx-reset") != -1 &&
          b.indexOf("jqx-splitter") != -1 &&
          b.indexOf("jqx-widget") != -1
        ) {
          throw "Invalid HTML Structure! Nested jqxSplitter cannot be initialized from a Splitter Panel. You need to add a new DIV tag inside the Splitter Panel and initialize the nested jqxSplitter from it!";
        }
      }
      if (
        this.host.parent().length > 0 &&
        this.host.parent()[0].className.indexOf("jqx-splitter") != -1
      ) {
        ...
        if (this.width == "100%" && this.height == "100%") {
          ...
          if (
            this.host.parent()[0].className.indexOf("jqx-splitter-panel") != -1
          ) {
      ...
      this._hasBorder =
        this.host.hasClass("jqx-hideborder") == false ||
        this.element.style.borderTopWidth != "";
      ...
      this._arrange();
      if (
        this.panels[0].collapsible == false &&
        this.panels[1].collapsible == false
      ) {
    ...
    _setTheme: function () {
      ...
      this.splitBar.addClass(
        this.toThemeProperty("jqx-splitter-splitbar-" + this.orientation)
      );
      this.splitBar.addClass(this.toThemeProperty("jqx-fill-state-normal"));
      this.splitBarButton.addClass(
        this.toThemeProperty("jqx-splitter-collapse-button-" + this.orientation)
      );
      this.splitBarButton.addClass(
    ...
    _arrange: function () {
      ...
      this._splitBarSize = !this._isTouchDevice
        ? this.splitBarSize
        : this.touchSplitBarSize;
    ...
    collapse: function () {
      ...
      if (b != -1) {
        ...
        this.splitBar.addClass(
          this.toThemeProperty("jqx-splitter-splitbar-collapsed")
        );
        this._layoutPanels();
    ...
    enable: function () {
      ...
      this.splitBarButton.removeClass(
        this.toThemeProperty("jqx-splitter-splitbar-collapsed")
      );
    },
    refresh: function (b) {
    ...
    _layoutPanels: function () {
      ...
      var l, r, e, u;
      var m = parseInt(this._splitBarSize);
      if (!this.showSplitBar) {
        m = 0;
      }
      var i = this.host[q]();
          ...
          if (j._hasBorder) {
            if (i - m == w) {
              if (j.orientation == "vertical") {
                ...
                j.splitBarButton[0].style.width =
                  parseInt(j._splitBarSize ) + "px";
              } else {
                j.splitBar[0].style.borderBottomColor = "transparent";
                j.splitBarButton[0].style.height =
                  parseInt(j._splitBarSize ) + "px";
              }
            } else {
              if (w == 0) {
    ...
    _raiseEvent: function (d, f) {
      ...
      b[0].size =
        this.orientation == "vertical"
          ? this.panel1[0].offsetWidth
          : this.panel1[0].offsetHeight;
      b[1].size =
        this.orientation == "vertical"
          ? this.panel2[0].offsetWidth
          : this.panel2[0].offsetHeight;
